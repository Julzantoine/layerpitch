-- LayerPitch — persistance des curseurs de paramètre (RTPC, 24/09).
-- track.fxSliders = [{ id, label, defaultValue, smoothSec, visible, bindings:[...], thresholds:[...] }], porté par le
-- MORCEAU entier. `tracks` est une table à colonnes explicites et upsert_track n'écrit que ces colonnes : sans nouvelle
-- colonne, les curseurs seraient perdus en silence à chaque publication (même piège que fx / fx_triggers, voir
-- 20260923010000). Purement additif : colonne avec défaut '[]', aucune donnée existante touchée ; upsert_track est
-- recréée en entier à partir de la version de 20260923010000, seul ajout : fx_sliders.

alter table public.tracks add column if not exists fx_sliders jsonb not null default '[]'::jsonb;
...
begin
  ...
  insert into public.tracks (id, owner_id, folder_id, title, description, mode, loopable, implementation_note,
    ...
    randomize_sections, layers, intro, outro, loops, sections, fx, fx_triggers, fx_sliders, updated_at)
  values (
    ...
    coalesce(nullif(payload->'fxSliders', 'null'::jsonb), '[]'::jsonb), now()
  )
  on conflict (id) do update set
    ...
    sections = excluded.sections, fx = excluded.fx, fx_triggers = excluded.fx_triggers, fx_sliders = excluded.fx_sliders, updated_at = now();
