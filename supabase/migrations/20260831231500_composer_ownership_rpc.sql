-- LayerPitch — les RPC upsert_* vérifient maintenant la propriété réelle (owner_id) au lieu de
-- l'admin unique en dur (is_admin()). Suite de 20260831231400 : is_admin() était explicitement
-- documenté comme un intérim "à remplacer une fois profiles/composer_profiles réellement
-- utilisés" (20260831110216) — c'est fait ici, is_admin() est donc retiré (plus aucun appelant).
--
-- Règle : create -> le compte appelant doit avoir un composer_profile (sinon "Non autorisé"),
-- devient propriétaire. update -> le owner_id existant doit correspondre au compte appelant
-- (sinon "Non autorisé : appartient à un autre compositeur"), owner_id ne change jamais après
-- création (absent des colonnes mises à jour dans le ON CONFLICT).

create or replace function public.current_composer_id()
returns uuid
language sql
stable
as $$
  select id from public.composer_profiles where profile_id = auth.uid();
$$;
...
begin
  ...
  select owner_id into v_existing_owner from public.ad_reels where id = v_ad_reel_id;
  if v_existing_owner is not null and v_existing_owner <> v_owner_id then
    raise exception 'Non autorisé : cet AdReel appartient à un autre compositeur';
  end if;
