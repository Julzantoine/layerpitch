-- LayerPitch — filtre de période sur le panneau Statistics admin (17 septembre).
--
-- Demande de Jules-Antoine : voir combien de nouveaux comptes/contenus sont apparus sur une
-- fenêtre récente (24h/7j/1mois/année), pas seulement les totaux depuis toujours. Décision
-- soumise avant de coder : les totaux existants restent inchangés (toujours "depuis toujours"),
-- le filtre ajoute une section "nouveaux sur la période" à côté plutôt que de recalculer tout le
-- panneau sur la période (les moyennes par compositeur perdraient leur sens sur une fenêtre
-- courte) — confirmé par Jules-Antoine.
