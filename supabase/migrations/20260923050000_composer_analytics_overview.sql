-- LayerPitch — analytique compositeur : vue d'ensemble dans le temps + détail par AdReel/pack
-- (23 septembre, demande de Jules-Antoine : page moins austère, liste des AdReels cliquables avec
-- leur analyse, courbes dans le temps, presets 24h / 7j / 1 mois / 6 mois / 1 an).
--
-- Aucune nouvelle table : tout est calculé à la lecture depuis analytics_events (déjà filtré par
-- propriétaire, gating par palier appliqué ICI, dans les fonctions -- un compte Free ne reçoit rien,
-- un Starter ne reçoit jamais lectures/morceaux/interactions, même en trafiquant l'appel).
--
-- Définitions : une VISITE = une session (session_id = un chargement de page public) ayant au moins
-- un événement sur la période ; elle est datée de son premier événement. Une LECTURE = un événement
-- track_play. (Le front public enregistre désormais aussi un événement page_open à l'ouverture, pour
-- que les visites sans interaction comptent -- avant, seule une interaction créait une session.)
--
-- Décision de grille (à confirmer par Jules-Antoine) : le filtre de période était réservé à Pro
-- (20260905010000) ; il est ici ouvert à Starter, borné à sa fenêtre de rétention (30 jours) -- ses
-- presets 24h/7j/1 mois fonctionnent, 6 mois et 1 an sont grisés côté écran (rétention insuffisante).
--
-- Fuseau : p_tz (défaut UTC) sert à aligner les jours/semaines sur l'heure locale du compositeur ;
-- les séries sont zéro-remplies (une période sans visite vaut 0, elle n'est jamais absente).
...
begin
  ...
  ent_out as (
    select jsonb_build_object(
      ...
      'name', coalesce(nullif(case when e.entity_type = 'adreel' then ar.label else pk.title end, ''), e.entity_id),
      'visits', e.visits,
...
-- ============================================================================
-- Détail d'un AdReel ou d'un pack
-- ============================================================================
...
begin
  ...
  if p_type is null or p_type not in ('adreel', 'pack') or p_id is null then
    raise exception 'p_type doit valoir adreel ou pack, et p_id est obligatoire';
  end if;
  if p_bucket is null or p_bucket not in ('hour', 'day', 'week', 'month') then
    raise exception 'p_bucket doit valoir hour, day, week ou month';
  end if;
