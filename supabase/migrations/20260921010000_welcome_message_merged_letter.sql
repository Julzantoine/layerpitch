-- LayerPitch — nouveau message d'accueil automatique (21 septembre) : fusion du message de
-- bienvenue de 20260911165000 et de la lettre aux bêta-testeurs envoyée le 20 septembre (via
-- admin_send_message, "comptes déjà existants uniquement", 20260920010000).
--
-- Retirés par rapport à la lettre, parce qu'un futur arrivant la lira des mois plus tard : le
-- nombre exact de testeurs ("10") et le remerciement "d'avoir répondu à l'appel". Les pays sont
-- conservés. Version française : la phrase "franglish" sur les tutos (20260911165000) est
-- conservée ; version anglaise : la blague sur l'accent français, volontairement différente (pour
-- un anglophone, les vidéos sont en anglais avec l'accent de Jules-Antoine -- ce n'est pas une
-- traduction de la phrase française).
--
-- Ne touche que les comptes qui terminent bienvenue.html APRÈS cette migration : les messages
-- d'accueil déjà insérés dans admin_messages ne sont pas modifiés.
