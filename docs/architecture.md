...
Champs par morceau : `mode` (`static`, `vertical`, `vertical-random`, `sequential`, `embranchement-vertical`), `loopable`, `startTrackBeat`, `maxLoops`, `normalizeVolume`, `bpm`/`beatsPerBar` pour les modes quantifiés. Le mode `sequential` accepte en plus des champs optionnels par emplacement (`segmentSlots[]`) : `nextOptions` (liste de `{targetId, label, transition}` — absence : comportement inchangé, avancement automatique ; présence : le visiteur choisit la suite par bouton au lieu d'un avancement automatique), `quantization` (`immediate`/`beat`/`bar`, défaut `bar` — quand la bascule choisie prend effet), `cutStyle` (`hard`/`fade`, défaut `fade` — comment l'emplacement source se termine à cet instant). `transition` (sur chaque entrée de `nextOptions`, pas sur l'emplacement) : fichier optionnel propre à CET embranchement précis (paire source→cible), joué entre la coupure et la cible, à la manière d'un Transition Object Wwise — absence : bascule directe. Voir `audio-engine.md` pour le détail du comportement.
