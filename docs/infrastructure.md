...
- **CORS requis sur le bucket** (trouvé au test visuel du 31 août, absent de cette décision initiale) : `player.js` charge l'audio via `fetch()` + `decodeAudioData` (Web Audio API, nécessaire au bouclage sample-accurate), soumis à la vérification CORS du navigateur dès que le site n'est pas sur le même domaine que `media.layerpitch.com` — contrairement aux images (`<img src>`, jamais concernées). Policy posée manuellement via le dashboard Cloudflare (R2 > bucket > Settings > CORS Policy) : `AllowedOrigins: ["*"]` — cohérent avec l'absence de presigned URLs déjà actée ci-dessus, et sans coût lié à l'origine (R2 egress-free). Le token R2 scope "Object Read & Write" ne suffit pas pour cette opération de niveau bucket (403) ; nécessite soit le dashboard, soit un token "Admin Read & Write". `AllowedMethods` étendue au fil des besoins réels plutôt que posée d'un coup : `GET`/`HEAD` à la migration initiale (31 août), `PUT` le même jour (pipeline d'upload backstage migré vers R2 dans la foulée, voir plus bas), `DELETE` le 1er septembre (suppression réelle des fichiers distants à la suppression d'un morceau/Sfx dans le backstage — voir `docs/LAYERPITCH_CHANGELOG.md`, entrée `[2026-09-01a]`).
...
- **Point de vigilance opérationnel** (implémentation, pas architecture) : la limite par défaut de 2 emails d'invitation/heure de Supabase nécessitera soit un SMTP personnalisé, soit un espacement des invitations lors du rollout bêta. **Confirmé en pratique le 31 août** (`email rate limit exceeded` reçu lors du test de `invite-tester`) — le point de vigilance était fondé, pas juste théorique.
...
**État réel d'implémentation des comptes multicasquettes** (Décision 4) : à vérifier avec Claude Code, non confirmé dans ce canal.
...
- **1er septembre** — le backstage supprime désormais réellement les fichiers R2 (`r2DeleteFile`, signature SigV4) quand un morceau/Sfx/couche/alternative/section/pool/boucle/transition est supprimé — point identifié et volontairement laissé de côté le 31 août (voir entrée ci-dessus), traité séparément. `AllowedMethods` du bucket étendue à `DELETE` (voir Décision 3). Testé réellement de bout en bout par Jules-Antoine (upload → lecture publique 200 → suppression → lecture publique 404). Détail complet : `docs/LAYERPITCH_CHANGELOG.md`, entrée `[2026-09-01a]`.
- **À trancher** : date d'ouverture réelle de la bêta A (dépend de l'essai à blanc + correctif cache) ; policy d'accès Zero Trust pour les repos testeurs (liste commune vs restriction par testeur) ; chiffres des quotas par palier (`plan_quotas`) ; ADR formalisant la révision de calendrier de la Partie B (documentée ici, ADR séparé à rédiger une fois ces docs poussées sur GitHub) ; nettoyage rétroactif ou non des fichiers déjà orphelins sur GitHub (morceaux/Sfx supprimés avant le 1er septembre, jamais nettoyés faute de `r2DeleteFile`) — pas tranché, risque de casser un vieux lien partagé si un fichier apparemment orphelin s'avère encore référencé ailleurs.
