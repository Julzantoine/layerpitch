...
async function runIndexInit(adreelId, adReels) {
  ...
  dom.window.LayerPitchSiteData = { loadSiteDataFromPostgres: () => Promise.resolve({ publishedAt: 1, adReels, customFonts: [] }) };
...
async function runPackInit(packId, packs) {
  ...
  dom.window.loadPostgresReadScripts = () => Promise.resolve();
  dom.window.LayerPitchSiteData = { loadSiteDataFromPostgres: () => Promise.resolve({ publishedAt: 1, packs, collections: [], adReels: [], customFonts: [] }) };
  await new Promise(resolve => setTimeout(resolve, 30));
  await dom.window.init();
  await new Promise(resolve => setTimeout(resolve, 30));
  return dom.window;
...
async function runCollectionInit(collId, collections) {
  ...
  dom.window.loadPostgresReadScripts = () => Promise.resolve();
  dom.window.LayerPitchSiteData = { loadSiteDataFromPostgres: () => Promise.resolve({ publishedAt: 1, collections, packs: [], adReels: [], customFonts: [] }) };
  await new Promise(resolve => setTimeout(resolve, 30));
  await dom.window.init();
  await new Promise(resolve => setTimeout(resolve, 30));
  return dom.window;
