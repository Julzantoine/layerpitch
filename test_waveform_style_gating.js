...
async function runIndexInit(adreelId, adReels, waveformStyle) {
  ...
  dom.window.LayerPitchSiteData = { loadSiteDataFromPostgres: () => Promise.resolve({ publishedAt: 1, adReels, customFonts: [], waveformStyle }) };
...
async function runPackInit(packId, packs, waveformStyle) {
  ...
  dom.window.loadPostgresReadScripts = () => Promise.resolve();
  dom.window.LayerPitchSiteData = { loadSiteDataFromPostgres: () => Promise.resolve({ publishedAt: 1, packs, collections: [], adReels: [], customFonts: [], waveformStyle }) };
  await new Promise(resolve => setTimeout(resolve, 30));
  await dom.window.init();
  await new Promise(resolve => setTimeout(resolve, 30));
  return dom.window;
...
async function runCollectionInit(collId, collections, waveformStyle) {
  ...
  dom.window.loadPostgresReadScripts = () => Promise.resolve();
  dom.window.LayerPitchSiteData = { loadSiteDataFromPostgres: () => Promise.resolve({ publishedAt: 1, collections, packs: [], adReels: [], customFonts: [], waveformStyle }) };
  await new Promise(resolve => setTimeout(resolve, 30));
  await dom.window.init();
  await new Promise(resolve => setTimeout(resolve, 30));
  return dom.window;
