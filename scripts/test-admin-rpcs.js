...
(async () => {
  ...
  try {
    ...
    try {
      await asProfile(client, NON_ADMIN_AUTH_ID, () => client.query("select set_platform_notice('devrait échouer')"));
    ...
    // ---- set_platform_notice() : chemin admin, vérifié en lecture brute ----
    try {
      await asProfile(client, adminId, () => client.query("select set_platform_notice('message de test — script automatisé')"));
      const { rows } = await client.query('select notice_message, notice_updated_at from platform_settings where id = true');
      check('set_platform_notice() écrit bien le message', rows[0].notice_message === 'message de test — script automatisé' && rows[0].notice_updated_at !== null);
